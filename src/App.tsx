...
const App: Component = () => {
  ...
  const playAux = (from: number, to: number) => {
    ...
    batch(() => {
      ...
      if (j >= 0) {
        ...
        setState("pieces", j, "type", type => type === 'C' ? 'c' : type);
  ...
  return (
    <>
      <div class="relative w-screen min-h-screen bg-main bg-contains flex flew-row items-center justify-around portrait:flex-col">
        ...
        <div class="flex flex-col bg-board b-cover p-6 border-2 border-black rounded-xl gap-4 z-20">
          ...
          <button class="btn" onClick={test}>Nouvelle partie</button>
